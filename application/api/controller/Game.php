...
use app\common\model\BingoBetTypes;
use app\common\model\BingoDraws;
...
class Game extends Controller
{
    ...
    public function getGameCurrentDraw()
    {
        ...
        return $this->success([
            ...
            'bingo_draw' => BingoDraws::getCurrentDraw(),
        ]);
    }
    ...
    public function getKenoGame()
    {
        try {
            // 获取所有玩法配置
            $betTypes = KenoBetTypes::order("sort asc")
                ->select();
     ...
     * 包括玩法配置和当前期数信息
     */
    public function getBingoGame()
    {
        try {
            // 获取所有玩法配置
            $betTypes = BingoBetTypes::order("sort asc")
                ->select();
            ...
            // 获取当前期数信息
            $currentDraw = BingoDraws::getCurrentDraw();
     ...
     * 分页获取已开奖的期数记录
     */
    public function getBingoDrawHistory()
    {
        try {
            $page = intval($this->params['page'] ?? 1);
            $limit = 10; // 每页10条
            ...
            // 获取已开奖的期数记录
            $draws = BingoDraws::where('status', 'in', [BingoDraws::STATUS_DRAWN, BingoDraws::STATUS_SETTLED])
                ->where('result_numbers', '<>', null)
                ->order('period_number desc')
                ->limit($offset, $limit)
                ->select();
            ...
            foreach ($draws as $draw) {
                $drawArray[] = [
                    ...
                    'draw_at' => $draw['draw_at'],
                    'status' => intval($draw['status']),
                    'status_text' => BingoDraws::getStatusText($draw['status'])
                ];
            }

            // 检查是否还有更多数据
            $totalCount = BingoDraws::where('status', 'in', [BingoDraws::STATUS_DRAWN, BingoDraws::STATUS_SETTLED])
                ->where('result_numbers', '<>', null)
                ->count();
     ...
     * 返回包含选择号码、开出号码、匹配结果的完整信息
     */
    public function getBingoBetHistory()
    {
        try {
            $page = intval($this->params['page'] ?? 1);
            $limit = 10; // 每页10条
            ...
            // 获取用户的投注记录
            $bets = BingoBets::where('user_id', $this->user['uuid'])
                ->where('merchant_id', $this->user['merchant_id'])
                ->order('created_at desc')
                ->limit($offset, $limit)
                ->select()
            ...
            // 检查是否还有更多数据
            $totalCount = BingoBets::where('user_id', $this->user['uuid'])
                ->where('merchant_id', $this->user['merchant_id'])
                ->count();
    ...
    /**
     * Bingo投注 (BCLC规则: 选择10个号码)
     * 使用Redis锁防止重复提交
     */
    public function placeBingoBet()
    {
        // 获取参数
        $selectedNumbers = $this->params['selected_numbers'] ?? [];
        $amount = floatval($this->params['amount'] ?? 0);
        ...
        // Redis锁的key
        $lockKey = sprintf(Bingo::BET_LOCK_KEY, $this->user['uuid']);
        $lockExpire = 3; // 锁定3秒
        ...
        try {
            ...
            // 查找当前可投注的期数（status=0）
            $currentDraw = BingoDraws::where('status', BingoDraws::STATUS_WAITING)
                ->field('id,period_number,status,end_at')
                ->where('end_at', '>', date('Y-m-d H:i:s', time() + 30))
                ->order('period_number desc')
                ->find();
            ...
            try {
                ...
                UserBalance::subUserBalance(
                    ...
                    $amount,
                    'game_bet',
                    'Bingo bet - Period: ' . $currentDraw['period_number'] . ' - Numbers: ' . implode(',', $selectedNumbers),
                    $bet->id
                );

                // 发送投注通知到 WebSocket (可选)
                // postData("http://127.0.0.1:8000/v1/game_api/bingo/bet", [
                //     'bet_amount' => $amount,
                //     'selected_numbers' => $selectedNumbers,
                //     'user_id' => $this->user['uuid'],
                //     'username' => $this->user['nickname'],

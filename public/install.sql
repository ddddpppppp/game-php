...
CREATE TABLE IF NOT EXISTS `game_user_balances` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `user_id` bigint(20) unsigned NOT NULL COMMENT '用户ID',
  `type` varchar(30) NOT NULL COMMENT '变动类型：gift-赠送, deposit-充值, deposit_gift-充值赠送, withdraw-提现, game_bet-投注, game_win-收益, withdraw_failed_refund-提现失败退款,game_bet_cancel-投注取消',
...
CREATE TABLE IF NOT EXISTS `game_user_frozen_balances` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `user_id` bigint(20) unsigned NOT NULL COMMENT '用户ID',
  `type` varchar(30) NOT NULL COMMENT '变动类型：game_bet-投注, gift-赠送, game_bet_cancel-投注取消',
  `amount` decimal(15,2) NOT NULL COMMENT '变动金额',
  `balance_before` decimal(15,4) NOT NULL COMMENT '变动前余额',
  `balance_after` decimal(15,4) NOT NULL COMMENT '变动后余额',
  `description` varchar(255) DEFAULT NULL COMMENT '描述',
